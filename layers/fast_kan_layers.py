...
from utils.utils import RadialBasisFunction, SplineLinear
...
class FastKANConvNDLayer(nn.Module):
    def __init__(self, conv_class, norm_class, input_dim, output_dim, kernel_size,
        ...
        self.spline_conv = nn.ModuleList([conv_class(self.grid_size * self.input_dim // self.groups,
                                                     self.output_dim // self.groups,
                                                     self.kernel_size,
                                                     self.stride,
                                                     self.padding,
        ...
        self.layer_norm = nn.ModuleList([norm_class(input_dim // groups, **filtered_norm_kwargs) for _ in range(groups)])

        self.rbf = RadialBasisFunction(self.grid_range[0], self.grid_range[1], self.grid_size)
